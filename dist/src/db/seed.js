...
function randomPrice() {
    return Number((Math.random() * 5000 + 100).toFixed(2));
}
async function seed() {
    console.time("seed");
    ...
    for (let batchStart = 0; batchStart < TOTAL_PRODUCTS; batchStart += BATCH_SIZE) {
        ...
        console.log(`Inserted ${Math.min(batchStart + BATCH_SIZE, TOTAL_PRODUCTS)}/${TOTAL_PRODUCTS}`);
    }
